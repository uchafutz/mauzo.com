            ...
            <div class="col-md-8">
                <div class="card" x-data="getState()" x-init="init({{ json_encode($inventoryItems) }}, {{ json_encode($units) }}, {{ isset($manufacturing) ? json_encode($manufacturing) : null }})">
                    ...
                    <div class="card-body">
                        ...
                        @isset($manufacturing)
                            <form action="{{ route('inventory.manufacturings.update', ['manufacturing' => $manufacturing]) }}"
                                method="POST" enctype="multipart/form-data">
                                @method('patch')
                            @else
                                <form action="{{ route('inventory.manufacturings.store') }}" method="POST"
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="config_unit_id"
                                        class="form-control @error('config_unit_id') is-invalid @enderror"
                                        x-model="form.config_unit_id">
                                        <option value="">Choose...</option>
                                        <template
                                            x-for="unit in units.filter(u => form.inventory_item_id ? u.unit_type_id == form.item.unit_type_id : true)">
                                            <option x-bind:value="unit.id" x-text="unit.name"
                                                :selected="form.config_unit_id === unit.id"></option>
                                        </template>
                                    </select>
                                ...
                                <x-form.custom-input name="quantity" type="number" label="Quantity" placeholder="Quantity"
                                    value="{{ isset($manufacturing) ? $manufacturing->quantity : null }}" />
                                <br />
